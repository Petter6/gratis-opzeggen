...
export default function RegistrationForm() {
  ...
  const fetchAddress = async () => {
    try {
      ...
      const res = await fetch(
        `/api/postcode?postcode=${postcode}&number=${huisnummer}`
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!verplichtIngevuld) {
      alert(
        "Vul alle verplichte velden in voordat je het formulier verstuurt."
      );
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} style={formContainer} lang="nl">
      ...
      <div style={formGroup}>
        <label style={label}>IBAN (Rekeningnummer) *</label>
        <input
          value={iban}
